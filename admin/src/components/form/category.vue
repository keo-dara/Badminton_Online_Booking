<template>
  <div>
    <n-card :title="isEditing ? $t('editNewCategory') : $t('addNewCategory')" class="dark-mode-card">
...
import { createCategories, updateCategories } from '~/api';
...
const emit = defineEmits(['category-added', 'category-updated'])
...
const createCategory = async (e: MouseEvent) => {
  ...
  if (await formRef.value?.validate()) {
    try {
      if (!props.category) {
        const result = await createCategories(newCategory.value.name);
